...
function collectText(...parts: any[]): string {
...
export function inferSuggestionFromApi(payload: {
  status?: number;
  message?: string;
  responseText?: string;
  responseBody?: any;
...
export function withSuggestion<T extends AnyObject>(err: T, context: 'invoice' | 'paymentLink'): T & { suggestion?: string } {
  const suggestion = inferSuggestionFromApi({
    status: (err as AnyObject).status,
    message: (err as AnyObject).message,
    responseText: (err as AnyObject).responseText,
    responseBody: (err as AnyObject).responseBody,
    context,
  });
  if (suggestion) {
    // Use Object.assign to preserve typing while merging suggestion in a controlled cast
    return Object.assign({}, err, { suggestion }) as T & { suggestion?: string };
  }
  return err as T & { suggestion?: string };
}
