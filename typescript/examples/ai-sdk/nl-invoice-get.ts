...
function parseId(text: string): string | undefined {
  const t = text.trim();
  const m1 = t.match(/\b([A-Z]{2}\d{4,})\b/); // e.g., NL697018
  if (m1) return m1[1];
...
async function main() {
  const text = process.argv.slice(2).join(' ') || 'get invoice NL123456';
  const id = parseId(text);
  if (!id) { console.error('Could not find an invoice id in your text. Try: "get invoice NL123456"'); process.exit(1); }
  ...
  if (!getInv) throw new Error('get_invoice tool not available');
  console.log(`Getting invoice ${id} on ${env}...`);
  const res = await (getInv as any).execute({ id });
  console.log(typeof res === 'string' ? res : JSON.stringify(res, null, 2));
}

main().catch((e) => { console.error('NL invoice get failed:', e?.message || e); process.exitCode = 1; });
