...
async function main() {
  const text = process.argv.slice(2).join(' ') || 'send invoice NL123456';
  const invoice_id = parseId(text);
  if (!invoice_id) { console.error('Could not find an invoice id in your text. Try: "send invoice NL123456"'); process.exit(1); }
  ...
  const sendInv = (tools as any)['send_invoice'];
  if (!sendInv) throw new Error('send_invoice tool not available');
  console.log(`Sending invoice ${invoice_id} on ${env}...`);
  const res = await (sendInv as any).execute({ invoice_id });
  console.log(typeof res === 'string' ? res : JSON.stringify(res, null, 2));
}

main().catch((e) => { console.error('NL invoice send failed:', e?.message || e); process.exitCode = 1; });
